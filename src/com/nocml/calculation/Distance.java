...
import com.nocml.pojo.TF;

import static com.nocml.calculation.Config.othre;
...
public class Distance {
	 ...
	 * 判断重叠区域，以剪枝
	 */
	public static boolean overlapThre(TF t1, TF t2) {
		Point maxmin = t1.getBL().max(t2.getBL());
		Point minmax = t1.getTR().min(t2.getTR());
		if(minmax.getX()-maxmin.getX()>=0 ||
			minmax.getY()-maxmin.getY()>=0){
			return false;
		}
		Point maxmax = t1.getTR().max(t2.getTR());
		Point minmin = t1.getBL().min(t2.getBL());

		Double MaxArea = (maxmax.getX()-minmin.getX())*(maxmax.getY()-minmin.getY());
		Double MinArea = (minmax.getX()-maxmin.getX())*(minmax.getY()-maxmin.getY());

		if(MinArea/MaxArea<= othre) {
			return false;
		}
		return true;
	...
	public static double dist(Line lp , Line tp)
	{
		...
		if(Lpar1>Lpar2)	
			...
			d_parallel=Lpar1;
//		System.out.println(d_parallel+d_perpen+angle);
